...
while running:  # 1. : 붙이는 경우 알아보기
    dt = clock.tick(60)  # 프레임에 따른 보정값 ()안에 값은 프레임수
    
    print("fps : "+str(clock.get_fps()))  #프레임수 확인
    print("dt : "+ str(dt))               #보정값 확인

    # 캐릭터가 1초에 100만큼 이동
    # 10fps : 1초에 10번 동작 - > 1번에 몇만큼 이동? 10만큼 이동
    # 20fps : 1초에 20번 동작 - > 1번에 몇만큼 이동? 5만큼 이동
    #프레임이 달라진다고 게임의 이동속도가 달라지면 안됨
